use iced::widget::{button, checkbox, column, horizontal_rule as hr, row, scrollable, slider, text, Space};
use iced::Element;
...
impl Manabu {
    pub fn view_settings(&self) -> Element<Message> {
        ...
        let theme_system = button(text("System").size(30.0 * self.settings.text_scale)).on_press(ChangedSettings::ThemeSystem.into()).style(if self.settings.theme != Theme::System { iced::widget::button::secondary } else { iced::widget::button::primary }).padding(10);
        let theme_light = button(text("Light").size(30.0 * self.settings.text_scale)).on_press(ChangedSettings::ThemeLight.into()).style(if self.settings.theme != Theme::Light { iced::widget::button::secondary } else { iced::widget::button::primary }).padding(10);
        let theme_dark = button(text("Dark").size(30.0 * self.settings.text_scale)).on_press(ChangedSettings::ThemeDark.into()).style(if self.settings.theme != Theme::Dark { iced::widget::button::secondary } else { iced::widget::button::primary }).padding(10);
